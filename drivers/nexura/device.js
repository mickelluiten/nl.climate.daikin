...
class NexuraDevice extends Device {
  onInit() {
    ...
    const spmode_config = settings.nexura_spmode;
...
*/
  ...
  refreshData() {
    ...
    const settings = this.getSettings();
    const nexura_ip = settings.nexura_ip;
    this.log('Nexura ip-address: ', nexura_ip);
    const nexura_interval = settings.nexura_interval || 10; // to prevent "undefined"...
  ...
  updateControlListeners(control_info, control_response) {
    ...
    const nexura_spmode = settings.nexura_spmode;
    this.log('Special mode set:', nexura_spmode);
...
*/
  ...
  daikinPowerControl(pow) {
    ...
    const settings = this.getSettings();
    const nexura_ip = settings.nexura_ip;
    const nexura_useGetToPost = settings.nexura_useGetToPost;
    const nexura_adapter = settings.nexura_adapter;
    let nexura_options = {};
    this.log('firmware < v2.0.1 (then useGetToPost):', nexura_useGetToPost);
    this.log('Adapter model:', nexura_adapter);

    if (nexura_useGetToPost) nexura_options = {
      useGetToPost: true,
    };
    else nexura_options = {
      useGetToPost: false,
    ...
    var daikin = new DaikinAC(nexura_ip, nexura_options, ((err) => {
