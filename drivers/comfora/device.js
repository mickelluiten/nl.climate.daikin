...
class ComforaDevice extends Device {
  onInit() {
    super.onInit();

    this.log('Comfora capability registration started...');
    const settings = this.getSettings();
    const spmode_config = settings.comfora_spmode;
    ...
    this.log('Comfora registration of Capabilities and Report Listeners completed!');
  ...
  refreshData() {
    ...
    if (this.comforaIsDeleted) {
      this.log('Comfora device has been deleted, the refresh loop is now stopped...');

      return;
    }

    const settings = this.getSettings();
    const comfora_ip = settings.comfora_ip;
    this.log('Comfora ip-address:', comfora_ip);
    const comfora_interval = settings.comfora_interval || 10; // to prevent "undefined"...
  ...
  updateControlListeners(control_info, control_response) {
    ...
    const comfora_spmode = settings.comfora_spmode;
  ...
  daikinPowerControl(pow) {
    ...
    const settings = this.getSettings();
    const comfora_ip = settings.comfora_ip;
    const comfora_useGetToPost = settings.comfora_useGetToPost;
    const comfora_adapter = settings.comfora_adapter;
    let comfora_options = {};
    this.log('firmware < v2.0.1 (then useGetToPost):', comfora_useGetToPost);
    this.log('Adapter model:', comfora_adapter);

    if (comfora_useGetToPost) comfora_options = {
      useGetToPost: true,
    };
    else comfora_options = {
      useGetToPost: false,
    };

    var daikin = new DaikinAC(comfora_ip, comfora_options, ((err) => {
